...
import { ALL, AnalyticsStore, FactsStore, FiltersStore, FilterUrl, format, NavStore } from '../core';
                            ...
                            >
                                <svg class="ic" [style.color]="s.color" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                    @switch (s.id) {
                                        ...
                                        @case ('sec-collab') {
                                            <path d="M9 13a5 5 0 0 0 7.5.5l3-3a5 5 0 0 0-7-7L11 5" />
                                            <path d="M15 11a5 5 0 0 0-7.5-.5l-3 3a5 5 0 0 0 7 7L13 19" />
                                        }
                                        @case ('sec-insights') {
                                        ...
                                        @case ('sec-compare') {
                                            ...
                                            <path d="M12 6 6 8l3 6a3 3 0 0 1-6 0l3-6z" />
                                            <path d="M12 6l6-2 3 6a3 3 0 0 1-6 0l3-6z" />
                                        }
                                    }
                                </svg>
                                {{ s.label }}
...
export class ShellComponent {
    ...
    private readonly savedFragment = decodeURIComponent(location.hash.slice(1));
    ...
    constructor() {
        ...
        // Reflect the section in view in the URL fragment (deep-linkable), while
        // leaving the filter query string untouched.
        effect(() => {
            const id = this.active();
            if (id && !this.showOnboarding()) {
                history.replaceState(null, '', `${location.pathname}${location.search}#${id}`);
            }
        });
        // Once the dashboard is actually on screen (data loads async, so this can be a
        // few ticks after construction), wire the scroll-spy and jump to the section the
        // user last left — so refreshing lands them in the same place.
        effect(() => {
            if (this.showOnboarding() || this.started) {
                return;
            }
            this.started = true;
            setTimeout(() => {
                ...
                if (this.savedFragment) {
                    this.active.set(this.savedFragment);
                    this.host.nativeElement.querySelector(`#${this.savedFragment}`)?.scrollIntoView({ block: 'start' });
                }
            });
        });
    }
